...
class ParsingContext:
    ...
    def get_current_object(self) -> Any:
        if len(self.parsing_stack) == 0 or self.parsing_stack[-1].state_type != StackStateType.IN_OBJECT:
            return None
    ...
    def add_value(self, value):
        ...
        if self.parsing_stack[-1].state_type == StackStateType.IN_OBJECT:
            self.parsing_stack[-1].current_value[self.parsing_stack[-1].current_key] = value
...
def parse_string(context: ParsingContext) -> Optional[ParseNextTokenResult]:
    ...
    if end_quote_pos == -1:
        ...
        return None
    ret = ParseNextTokenResult(JsonTokenType.STRING, context.json_str[context.position + 1:end_quote_pos])
...
def parse_number(context: ParsingContext) -> Optional[ParseNextTokenResult]:
    ...
    if has_decimal_point or has_exponent:
        ret = ParseNextTokenResult(JsonTokenType.NUMBER, float(context.json_str[context.position:end_pos]))
    else:
        ret = ParseNextTokenResult(JsonTokenType.NUMBER, int(context.json_str[context.position:end_pos]))
    context.position = end_pos
    return ret


def skip_whitespace(context: ParsingContext):
    while context.position < len(context.json_str) and context.json_str[context.position].isspace():
        context.position += 1
...
def parse_incomplete_json(context: ParsingContext) -> Optional[Dict[str, Any]]:
    while True:
        ...
        if result is None:
            ...
            if len(context.parsing_stack) == 0 or context.parsing_stack[-1].state_type != StackStateType.IN_OBJECT:
            ...
            if len(context.parsing_stack) == 0 or context.parsing_stack[-1].state_type != StackStateType.IN_ARRAY:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(
            ...
            result.string_value if result.string_value is not None else (
                result.number_value if result.number_value is not None else ""))
