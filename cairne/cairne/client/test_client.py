...
def temporary_world(name: str):
    ...
    delete_request = worlds_schema.DeleteWorldRequest(
...
def find_world(worlds: List[worlds_schema.World], world_id: str) -> Optional[worlds_schema.World]:
    for world in worlds:
...
def test_characters():
    with temporary_world(name="Test World") as world:
        ...
        response = requests.post(f"{HOST}/world/{world.world_id}/characters", json=json.loads(request.model_dump_json()))
        assert response.status_code == 200
        create_response = characters_schema.CreateCharacterResponse(**response.json())
        
        response = requests.get(f"{HOST}/world/{world.world_id}/characters/{create_response.character_id}")
        assert response.status_code == 200
        ...
        response = requests.delete(f"{HOST}/world/{world.world_id}/characters/{create_response.character_id}")
        assert response.status_code == 200
