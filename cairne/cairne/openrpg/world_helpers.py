...
def get_theme(world: generated_model.GeneratedEntity) -> Optional[str]:
	generated_theme = typing.cast(generated_model.GeneratedString, world.children["theme"])
	return generated_theme.parsed


def describe_existing_characters(world: generated_model.GeneratedEntity) -> str:
	result = []
	characters_dict = typing.cast(generated_model.EntityDictionary, world.children["characters"])
	for incomplete in characters_dict.entities.values():
		generated_name = typing.cast(generated_model.GeneratedString, incomplete.children["name"])
		generated_faction = typing.cast(generated_model.GeneratedString, incomplete.children["faction"])
		name = (
...
class CharacterCounts(BaseModel):
	...
	def format_count_instructions(self, target: CharacterGenerationTarget) -> List[str]:
		...
		for faction, count in self.per_faction.items():
			if count < target.number_of_characters_per_faction:
				...
				instructions.append(f"You need to create {remaining} more characters of faction {faction}.")
			else:
				instructions.append(f"You do not have to create any more characters for faction {faction}.")
		
		target_total = target.number_of_total_characters - self.total
		if target_total > 0:
			instructions.append(f"You need to generate at least {target_total} more characters in total.")
		return instructions

	def get_incomplete_faction_names(self, target: CharacterGenerationTarget) -> List[str]:
		result = []
		for faction, count in self.per_faction.items():
			if count < target.number_of_characters_per_faction:
	...
	@staticmethod
	def get_character_counts(world: generated_model.GeneratedEntity) -> "CharacterCounts":
		factions = get_factions(world)
		ret = CharacterCounts(
	  		per_faction={
			faction: 0 for faction in factions
		})
