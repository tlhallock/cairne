...
def parse_string(context: ParseContext, specification: spec.ValueSpecification, raw: Any) -> gen.GeneratedString:
...
def parse_float(context: ParseContext, specification: spec.ValueSpecification, raw: Any) -> gen.GeneratedFloat:
...
def parse_integer(context: ParseContext, specification: spec.ValueSpecification, raw: Any) -> gen.GeneratedInteger:
...
def parse_boolean(context: ParseContext, specification: spec.ValueSpecification, raw: Any) -> gen.GeneratedBoolean:
    ...
    if isinstance(raw, bool):
        ...
        if raw.lower() in ["1"] + prefixes("true") + prefixes("yes"):
            normalized = True
        elif raw.lower() in ["0"] + prefixes("false") + prefixes("no") + prefixes("null") + prefixes("none"):
            normalized = False
...
def get_child_input(context: ParseContext, dict_value: Optional[Dict[str, Any]], key: str) -> Any:
...
def parse_object(context: ParseContext, specification: spec.ObjectSpecification, raw: Any) -> gen.GeneratedObject:
    ...
    if specification.parser.parser_name == spec.ParserName.ENTITY:
        ...
        raise NotImplementedError(f"Unknown parser name: {specification.parser.parser_name}")


def parse_entity_dictionary(context: ParseContext, specification: spec.EntityDictionarySpecification, raw: Any) -> gen.EntityDictionary:
    ...
    for key, child_input in raw.items():
        ...
        with context.with_path(key=f'["{uuid}"]'):
            untyped_child = parse(context, specification.entity_specification.parser, child_input)
...
def parse_list(context: ParseContext, specification: spec.ListSpecification, raw: Any) -> gen.GeneratedList:
    parsed = parse_js_list(context, raw)
    if parsed:
        elements = []
        for index, raw_element in enumerate(parsed):
            with context.with_path(key=f"[{index}]"):
                element = parse(context, specification.element_specification.parser, raw_element)
...
def parse(context: ParseContext, specification: spec.ParserSpecification, raw: Any) -> gen.Generated:
