...
class EntityType(str, Enum):
    ...
    def get_dictionary_path(self) -> GeneratablePath:
        return GeneratablePath(
            path_elements=[GeneratablePathElement(key=self.get_field_name())]
        )
...
class GeneratableSpecification(BaseModel):
    ...
    generation: GenerationSpecification = Field(
        default_factory=lambda: GenerationSpecification()
    )
...
class ListSpecification(GeneratableSpecification):
    ...
    def create_example(self) -> Any:
        ...
        if num_examples is None:
            num_examples = 2
        return [
            self.element_specification.create_example() for _ in range(num_examples)
        ]
