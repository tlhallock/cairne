...
def list_entities(world_id: uuid.UUID, entity_type: str) -> generated_schema.ListEntitiesResponse:
    ...
    command = generated_commands.ListEntities(datastore=datastore, user="test", request=request)
    response = command.execute()
    return response
...
def get_world(world_id: uuid.UUID) -> generated_schema.GetEntityResponse:
    ...
    command = world_commands.GetWorld(datastore=datastore, user="test", world_id=world_id)
    response = command.execute()
    return response
...
@cross_origin(origins=['*']) 
@validate()
def get_entity(world_id: uuid.UUID, entity_id: uuid.UUID) -> generated_schema.GetEntityResponse:
    logger.info("Get entity", world_id=world_id, entity_id=entity_id)
    command = generated_commands.GetEntity(datastore=datastore, user="test", world_id=world_id, entity_id=entity_id)
    response = command.execute()
    return response
...
def create_entity(
    ...
    logger.info("Create entity", world_id=world_id, entity_type=entity_type, request=body)
