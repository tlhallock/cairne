...
class OpenAIService:
	...
	@classmethod
	def translate_message(cls, message: generate_model.GenerationChatMessage) -> Dict[str, Any]:
		return {
			"role": message.role.value,
			"content": message.message,
		}
	
	def generate_json(self, generation: generate_model.Generation) -> OpenAIGenerationResult:
		kwargs = dict(
			model="gpt-3.5-turbo-1106",  # TODO: This is set on the generation
			messages=[self.translate_message(message) for message in generation.prompt_messages],
		...
		logger.info("Generated JSON:", generation_time=generation_time, finish_reason=finish_reason, content=content)
...
def run_openai_generation(generation: generate_model.Generation):
	try:
		...
		logger.exception("Error running OpenAI generation", generation_id=generation.generation_id, excepttion=e)
